  ...
  return (
    <div className="min-h-screen font-mono bg-gradient-to-br from-black to-red-900 text-white flex flex-col md:flex-row">
      ...
      <div className="flex justify-between items-center p-4 bg-black md:hidden border-b border-yellow-500">
        ...
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-yellow-400">
      ...
      >
        <div>
          <h2 className="text-2xl font-extrabold mb-6 text-yellow-400 hidden md:block">🎰 Game Admin</h2>
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        <h2 className="text-3xl font-bold text-yellow-300 mb-6">🎮 Games by Agent</h2>

        {loading && <p className="text-yellow-300">⏳ Loading...</p>}
        {error && <p className="text-red-500">❌ {error}</p>}

        {!loading && fetched && players.length === 0 && (
          <p className="text-pink-400 text-2xl">😕 No player data found for this agent.</p>
        )}
