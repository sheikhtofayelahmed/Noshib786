...
const GamerSummary = () => {
    ...
    const fetchWaitingPlayers = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/getWaitingPlayersByGamerId`, // No query params in URL
          {
            method: "POST", // Specify POST method
            headers: {
              "Content-Type": "application/json", // Tell server we're sending JSON
            },
            body: JSON.stringify({ gamerId }), // Send gamerId in the request body as JSON
          }
        );
  ...
  }, [gamerId]); // Re-run effect if agentId changes
  ...
  const calculateDeductedTotal = (players) => {
    const base = players.reduce(
      (sum, p) => {
...
<h2 style="
  ...
  const { OneD, TwoD, ThreeD, total } = calculateDeductedTotal(waitingPlayers);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-black via-gray-900 to-black text-white font-mono">
        <div className="bg-yellow-100 text-yellow-800 border border-yellow-300 rounded-lg p-6 shadow-lg max-w-md text-center">
          ...
          <p className="text-sm">
            There are no gamer entries available at the moment.
          </p>
        </div>
      </div>
    );
        ...
        <div className="text-center text-white font-mono text-lg mt-2">
          <div className="inline-block bg-gradient-to-r from-yellow-500 via-red-600 to-yellow-500 px-4 py-2 rounded-full shadow-[0_0_15px_rgba(255,215,0,0.6)] border border-yellow-300">
            <span className="mx-2 text-black font-extrabold tracking-wider">
              🎯 1D: <span className="text-white">{OneD.toFixed(1)}</span>
            </span>
            <span className="mx-2 text-black font-extrabold tracking-wider">
              🎯 2D: <span className="text-white">{TwoD.toFixed(1)}</span>
            </span>
            <span className="mx-2 text-black font-extrabold tracking-wider">
              🎯 3D: <span className="text-white">{ThreeD.toFixed(1)}</span>
            </span>
          </div>
        </div>
      </h2>
      <div ref={contentAllRef} className="mt-8 max-w-4xl mx-auto space-y-12">
        <button
          ...
          <React.Fragment key={idx}>
            ...
            >
              ...
              <h2 className="text-lg font-bangla font-semibold text-center mb-1 ">
                Player: {player.name || ""} || Sub Agent: {player.SAId || ""}
              </h2>
